...
export default function Testimonials() {
  ...
  useEffect(() => {
    ...
    gsap.fromTo(sectionRef.current.querySelectorAll(".quote-item"),
      { opacity: 0, y: 30 },
  ...
  return (
    <section ref={sectionRef} className="py-24 md:py-36 px-6 md:px-14 bg-white">
      <div className="max-w-7xl mx-auto">
        <p className="text-label text-amber mb-16">What Clients Say</p>

        <div className="space-y-16 md:space-y-20">
          {quotes.map((q, i) => (
            <div key={i} className="quote-item opacity-0">
              <p className="font-display text-navy leading-[1.4] mb-8 max-w-4xl"
                style={{ fontSize: "clamp(1.2rem, 2.2vw, 1.6rem)", fontWeight: 500 }}>
                &ldquo;{q.quote}&rdquo;
              </p>
              <div className="flex items-center gap-6 pt-6 border-t border-border">
                <div className="w-9 h-9 rounded-full bg-blue flex items-center justify-center flex-shrink-0">
                  <span className="font-display font-bold text-white text-xs">{q.name.charAt(0)}</span>
                </div>
                <div>
