        ...
        <div>
          {services.map((s, i) => (
            <div key={i} ref={(el) => { rowRefs.current[i] = el; }} className="opacity-0">
              <button
                onClick={() => toggle(i)}
                className="w-full border-t border-border pt-7 pb-7 flex items-baseline gap-5 md:gap-8 text-left group"
              >
                <span className="text-label text-text-dim w-7 flex-shrink-0 mt-1 group-hover:text-navy transition-colors">
                  {s.n}
                </span>
                <span className={`text-heading font-display flex-1 leading-[1] transition-colors duration-200 ${open === i ? "text-blue" : "text-navy group-hover:text-blue"}`}>
                  {s.title}
                </span>
                <span className="font-display font-medium text-sm text-amber hidden md:block flex-shrink-0 mb-1">
                  {s.pricing[0].price}
                </span>
                <span className={`flex-shrink-0 w-8 h-8 rounded-full border border-border flex items-center justify-center transition-all duration-300 mb-1 group-hover:border-navy ${open === i ? "bg-navy border-navy rotate-45" : ""}`}>
              ...
              <div className={`overflow-hidden transition-all duration-500 ease-in-out ${open === i ? "max-h-[28rem] opacity-100" : "max-h-0 opacity-0"}`}>
                <div className="pb-10 pl-12 md:pl-[3.75rem] grid md:grid-cols-[1fr_auto] gap-8 items-start">
                  <p className="text-body-lg text-text-muted max-w-xl leading-relaxed">
                    {s.description}
                  </p>
                  <div className="bg-bg rounded-lg p-5 min-w-[220px] border border-border">
